...
export function RunCommand({ commandCode }: RunCommandProps) {
  ...
  return (
    <button
      className="shrink-0 justify-center text-sm font-medium transition-colors hover:bg-white/90 dark:hover:bg-white/5 h-8 flex items-center text-zinc-600 dark:text-zinc-50 shadow-none bg-white dark:bg-neutral-800 w-full px-3 py-1.5 rounded-lg border border-zinc-700"
      onClick={async () => {
        if (commandCode) {
          setIsCopied(!isCopied);
          await copyToClipboard("npx reverse add");
        }
      }}
    >
